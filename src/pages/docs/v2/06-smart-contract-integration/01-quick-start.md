...
interacts with the Rsk Swap Protocol using one specific set of tools (`truffle` + `npm` + `mocha`).
...
In order to reference the Rsk Swap V2 contracts, you should use the NPM artifacts we deploy containing the core and
...
[`@thinkanddev/rskswap-core`](https://www.npmjs.com/package/@thinkanddev/rskswap-core) and 
[`@thinkanddev/rskswap-periphery`](https://www.npmjs.com/package/@thinkanddev/rskswap-periphery) packages.

```shell script
npm i --save @thinkanddev/rskswap-core
npm i --save @thinkanddev/rskswap-periphery
```

If you check the `node_modules/@thinkanaddev` directory, you can now find the RsksWap V2 contracts. 

```shell script
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@thinkanddev/rskswap-core/contracts
UniswapV2ERC20.sol    UniswapV2Pair.sol     libraries/
UniswapV2Factory.sol  interfaces/           test/
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@thinkanddev/rskswap-periphery/contracts/
...
```solidity
import '@thinkanddev/rskswap-periphery/contracts/libraries/UniswapV2Library.sol';
...
Note you should only deploy the precompiled Rsk Swap contracts in the `build` directories for unit tests. 
...
This is problematic because in Rsk Swap we use the hash of the bytecode in the v2-periphery
[`UniswapV2Library`](https://github.com/Think-and-Dev/rskswap-periphery/blob/master/contracts/libraries/UniswapV2Library.sol#L24),
to compute the pair address.

To get the bytecode for deploying UniswapV2Factory, you can import the file via:

```javascript
const UniswapV2FactoryBytecode = require('@@thinkanddev/rskswap-core/build/UniswapV2Factory.json').bytecode
```
